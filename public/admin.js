...
(function () {
  ...
  */
  ...
  /* Customer Notes + Note Suggestions tabs moved to
     public/admin/tab-customer-notes.js (Phase 6). Boot wires them via
     window.__pioneerAdmin.tabs.customerNotes.init() +
     window.__pioneerAdmin.tabs.noteSuggestions.init(). The auth-state
     refresh calls .refresh() on each.

     customerLabelForSlug is kept here (separate from the tab module's
     internal copy) because Service Recoveries below still depends on
     it. Will consolidate when Service Recoveries is extracted. */
